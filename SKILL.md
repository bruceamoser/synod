...
1. `python3 $SKILL/scripts/council.py judge-brief <run>`. The engine assembles `judge/brief.json` from ledger fields only (field whitelist: speaker (anonymized role), stance, argument, confidence, rebutting, evidence as `{source, claim}` — never verbatim excerpts, never a role name) and wall-lints it against the problem statement, `problem.md`, and every raw source. It also writes `judge/speaker_map.json` (speaker → role) for council-side traceability — that file must never reach the judge subagent.
